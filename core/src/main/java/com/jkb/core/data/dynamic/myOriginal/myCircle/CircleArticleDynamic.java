...
public class CircleArticleDynamic extends DynamicInCircleDynamic {
    ...
    public CircleArticleDynamic(DynamicCircleListEntity.DataBean dataBean) {
    ...
    private void handleArticleDynamicData(DynamicCircleListEntity.DataBean dynamic) {
        ...
        List<DynamicCircleListEntity.DataBean.ContentBean.ArticleBean> reqArticles
                = content.getArticle();
        if (reqArticles == null || reqArticles.size() == 0) {
            isData_effective = false;
            return;
        }
        articleInfo = new DynamicContentArticleInfo();
        List<DynamicContentArticleInfo.Article> articles = new ArrayList<>();
        for (DynamicCircleListEntity.DataBean.ContentBean.ArticleBean reqArticle :
                reqArticles) {
